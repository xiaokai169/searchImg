...
# ========== 共识重排（手机实拍的核心防线） ==========
# 现象：手机拍照搜索时结果里混入不同品类的商品。
#
# 实测（images/ 下 4 张手机实拍跑真实链路 top-100）：「正确品类稳定出现在 top-15 内，
# 且数量充足」—— 097e 无人机 27/61 张排在 1,2,3,5,7,8…，459b 无人机 12/92 张排在
# 4,6,10,13…，e18c 无人机 24/99 张排在 1,4,6,7…。所以瓶颈是【头部排序被跨域噪声
# 污染】（错误品类挤进前 3），不是召回不足，也不是库覆盖率。
#
# 上面两道防线对手机实拍都失效，且都不该简单打开：
#   品类过滤 —— AUTO_CATEGORY_FILTER 已关闭；而 category 字段本身不可靠，查库实测：
#              23 张无人机散在 4 个品类、105 张椅子散在 3 个品类，且
#              'Lighting & Electrical'(228) 与 'Lights & Lighting'(11) 是同义重复类目。
#              拿它过滤会把同一台无人机的不同白底图判成不同品类给砍掉。
#   共识过滤 —— NAME_ANCHOR_MIN_TOP_SCORE=0.80 的门控对手机实拍直接跳过；且它的基准是
#              top1 单条，而 459b 的 top1 是 'Office chair'（错），拿错锚当基准会砍光正确答案。
#
# 解法：用 top-K 结果集的【词频共识】重排。共识来自群体投票，对单个错误 top1 免疫 ——
# 459b 的 top20 里 'drone' 出现 11 次而 'chair' 只 1 次，投票能识别真品类。
# 信号优先用 keywords_en / keywords_cn（98.8% 覆盖的商品词表，此前在检索链路里只写不读），
# 再叠加 product_name 分词。
#
# ⚠️ 只重排、不过滤：结果集不会因为重排而变空。库中确实没有对应品类时，
#    仍会把"视觉上最像的"顶上来，由前端如实展示相似度百分比供用户判断。
CONSENSUS_RERANK_ENABLE = True

# 参与投票的候选条数。取 20：实测正确答案都在 top-15 内，20 能覆盖到且不至于
# 把长尾噪声也拉进投票稀释共识。
CONSENSUS_RERANK_TOP_N = 20

# λ（满共识覆盖时的最大加分）。
# 下限：459b 中正确的无人机 #4 原始 0.585，错误的 Office chair #1 是 0.648，
#       需要 λ > 0.063 才能翻盘。
# 上限：λ 太大会把分数顶到虚高 —— λ=0.35 时 top1 变成 1.000，前端显示"100% 相似度"，
#       误导用户以为完全一致。
#
# eval_rank.py 实测扫描（4 张手机实拍，指标 = top1 是否正确 / 前 5 条纯度）：
#   λ=0    3/4  13/20      λ=0.15  4/4  19/20  ← 取此值：达到最优的最小 λ
#   λ=0.10 4/4  18/20      λ=0.20  4/4  19/20
#                          λ=0.25  4/4  19/20
#                          λ=0.35  4/4  19/20  （无额外收益，仅分数虚高）
# 从 0.15 起纯度已饱和，再大只损失分数可信度，故取饱和点的最小值。
CONSENSUS_RERANK_WEIGHT = 0.15

# 共识词门槛：一个词至少覆盖 15% 的候选、且出现在 ≥2 条结果里，才算品类共识词。
# 两道门槛同时用：比例阈值随候选数自适应，绝对下限防止候选极少时单词垄断。
CONSENSUS_MIN_DF_RATIO = 0.15
CONSENSUS_MIN_DF = 2
